...
create or replace function public.save_portfolio_project_video(
...
begin
  ...
  if p_project_id is null then
    insert into public.projects (request_id, created_by, title, slug, description, location, year, status, youtube_video_id)
    values (p_request_id, v_user, btrim(p_project->>'title'), p_project->>'slug',
      ...
      (p_project->>'year')::integer, p_project->>'status', nullif(p_project->>'youtube_video_id', '')) returning id into v_id;
  else
    v_id := p_project_id;
    ...
    update public.projects set title = btrim(p_project->>'title'), description = btrim(coalesce(p_project->>'description', '')),
      ...
      status = p_project->>'status', youtube_video_id = nullif(p_project->>'youtube_video_id', ''), request_id = p_request_id, version = version + 1, updated_at = now()
      where id = v_id;
...
$$;
revoke all on function public.save_portfolio_project_video(integer,integer,uuid,jsonb,jsonb) from public, anon, authenticated;
grant execute on function public.save_portfolio_project_video(integer,integer,uuid,jsonb,jsonb) to authenticated;
